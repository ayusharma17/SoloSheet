...
import { UploadCleanupJournal, retryDisposition } from "../src/lib/upload-lifecycle.ts";
...
test("explicit recovery clears recent temporary uploads", async () => {
  const userId = "70000000-0000-4000-8000-000000000001";
  const sessionId = "71000000-0000-4000-8000-000000000001";
  const fileName = "72000000-0000-4000-8000-000000000001.pdf";
  const calls = [];
  const client = {
    auth: { getUser: async () => ({ data: { user: { id: userId } }, error: null }) },
    rpc: async (name, args) => {
    ...
    storage: {
      from: () => ({
        list: async prefix => prefix === userId
          ? { data: [{ name: sessionId }], error: null }
          : { data: [{ name: fileName, updated_at: new Date().toISOString() }], error: null },
        remove: async paths => {
          calls.push({ kind: "remove", paths });
          return { error: null };
        },
  ...
  assert.equal(await cleanupAllCourseUploads(client, userId), 1);
  assert.deepEqual(calls[0], {
    kind: "remove",
    paths: [`${userId}/${sessionId}/${fileName}`],
  });
  assert.equal(calls[1].name, "release_course_material_uploads");
});
