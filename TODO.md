...
- [ ] Make preserved uploads genuinely recoverable across modal closure, refresh, sign-out, and deployment. Persist a server-backed upload/extraction session containing the request ID and owned storage paths (never signed URLs), recover it automatically without exposing request-management concepts to the user, refresh signed URLs when needed, and reuse the original idempotent request without charging twice. Successful, explicitly failed, expired, and discarded sessions must release their objects and quota deterministically. The manual **Clear saved temporary uploads** action is only an emergency escape hatch, not completion of this item.
- [ ] Simplify failed-generation UX: remove **Resume status**, request-ID recovery language, double-charge explanations, and other internal job-state controls from the upload modal. Show a plain failure message with a single **Try again** action. Behind that action, automatically reconcile/reuse any ambiguous existing request so an accepted job is not duplicated or charged twice; if the request definitely never started or reaches a terminal failure, refund exactly once and delete its temporary uploads before allowing a fresh attempt.
